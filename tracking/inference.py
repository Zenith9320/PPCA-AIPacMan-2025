...
def constructBayesNet(gameState: hunters.GameState):
    ...
    MAX_OBSERVE = X_RANGE + Y_RANGE + MAX_NOISE - 2
    for i in range(0, MAX_OBSERVE + 1):
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for item in eliminationOrder:
            factors, temp = joinFactorsByVariable(factors, item)
            if len(temp.unconditionedVariables()) > 1:
                factor = eliminate(temp, item)
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        "*** YOUR CODE HERE ***"
        tot = sum(self.values())
        dic = {k: v / tot for k, v in self.items()}
        random_value = random.random()

        probability = 0.0
        for key, pro in dic.items():
            probability += pro
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        return busters.getObservationProbability(noisyDistance, ghostDistance)
        raiseNotDefined()
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        position_num = len(self.legalPositions)

        for i in range(self.numParticles):
            pos = self.legalPositions[i % position_num]
